...
void Operaciones::FingerCount(void){
...
Operaciones::Operaciones(void){
	...
	if (!face_cascade.load(faceClassifierFileName))
		throw runtime_error("can't load file " + faceClassifierFileName);
}

void Operaciones::calibrate(Mat src){
    cvtColor(src, background, COLOR_BGR2GRAY);
    calibrated = true;
}

Mat Operaciones::getForeground(Mat src){
    Mat foregroundMask = getForegroundMask(src);
    Mat foreground;
    src.copyTo(foreground, foregroundMask);
    return foreground;
}

Mat Operaciones::getForegroundMask(Mat src) {
	Mat foregroundMask;
	if (!calibrated) {
		foregroundMask = Mat::zeros(src.size(), CV_8UC1);
		return foregroundMask;
	}
	cvtColor(src, foregroundMask, COLOR_BGR2GRAY);
	removeBackground(foregroundMask, background);
	return foregroundMask;
}

void Operaciones::removeBackground(Mat src, Mat background){
    int thresholdOffset = 10;

	for (int i = 0; i < src.rows; i++) {
		for (int j = 0; j < src.cols; j++) {
			uchar framePixel = src.at<uchar>(i, j);
...
Mat Operaciones::conversorCIELab(Mat src){
    Mat resultado = src.clone();
    cvtColor(src, resultado, COLOR_BGR2Lab);
    return resultado;
}

void Operaciones::removeFaces(Mat src, Mat output){
    vector<Rect> faces;
	Mat frameGray;

	cvtColor(src, frameGray, COLOR_BGR2GRAY);
	equalizeHist(frameGray, frameGray);

	face_cascade.detectMultiScale(frameGray, faces, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, Size(120, 120));
...
Rect getFaceRect(Mat src){
    vector<Rect> faceRectangles;
    Mat inputGray;

	cvtColor(src, inputGray, COLOR_BGR2GRAY);
	equalizeHist(inputGray, inputGray);

	face_cascade.detectMultiScale(inputGray, faceRectangles, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, Size(120, 120));
...
Mat Operaciones::findFingersCount(Mat input_image, Mat frame) {
...
double Operaciones::findPointsDistance(Point a, Point b) {
	Point difference = a - b;
	return sqrt(difference.ddot(difference));
}

vector<Point> Operaciones::compactOnNeighborhoodMedian(vector<Point> points, double max_neighbor_distance) {
...
double Operaciones::findAngle(Point a, Point b, Point c) {
...
bool Operaciones::isFinger(Point a, Point b, Point c, double limit_angle_inf, double limit_angle_sup, Point palm_center, double min_distance_from_palm) {
...
vector<Point> Operaciones::findClosestOnX(vector<Point> points, Point pivot) {
...
double Operaciones::findPointsDistanceOnX(Point a, Point b) {
...
void Operaciones::drawVectorPoints(Mat image, vector<Point> points, Scalar color, bool with_numbers) {
	for (int i = 0; i < points.size(); i++) {
...
void Operaciones::drawSkinColorSampler(Mat src) {
	int frameWidth = src.size().width, frameHeight = src.size().height;
...
Mat Operaciones::getSkinMask(Mat src) {
	Mat skinMask;
	...
	Mat hsvInput;
	cvtColor(src, hsvInput, CV_BGR2HSV);
...
void Operaciones::performOpening(Mat binaryImage, int kernelShape, Point kernelSize) {
